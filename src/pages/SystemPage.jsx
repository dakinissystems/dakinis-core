...
import { DAKINIS_BUSINESS_SLUG_BY_VERTICAL, DAKINIS_ENTITY_BY_VERTICAL } from "../catalog/business-mapping.js";
import {
...
export default function SystemPage({ activeSystemKey, navigate }) {
  ...
  const systemPageContent = DAKINIS_SYSTEM_PAGE_CONTENT[activeSystemKey] || DAKINIS_SYSTEM_PAGE_CONTENT.clinica;
  ...
  useEffect(() => {
    ...
    async function dakinisLoadRecords() {
      setRecordsError("");
      try {
    ...
    dakinisLoadRecords();
    return () => controller.abort();
  }, [activeSystemKey, apiSession, entityName, tenantSlugForVertical]);
  ...
  async function dakinisHandleMockSubmit(event) {
    ...
    try {
      ...
      const json = await dakinisTenantJsonFetch(
        `/api/tenant/mock-records?entity=${encodeURIComponent(entityName)}`,
        apiSession,
        { businessId: tenantSlugForVertical, businessTypeHeader: activeSystemKey }
      );
      ...
      setRecordsSynced(false);
      setRecordsError((prevE) => prevE || "Guardado solo en local hasta que la API este disponible");
    }
  }
    ...
    <section className="modules">
      <div className="container">
        ...
        <p className="lead">
          ...
          {configStatus === "loading" ? "cargando..." : configStatus === "success" ? "tenant + tipo" : "fallback local"}
        ...
        {recordsError ? (
          ...
          <p className="lead">Registros leidos desde SQLite para este tenant y entidad <code>{entityName}</code>.</p>
        ) : null}
        ...
        <form className="mockup-form card" onSubmit={dakinisHandleMockSubmit}>
          {activeMockup.formFields.map((field) => (
            <label className="mockup-field" key={`${activeSystemKey}-${field.key}`}>
              ...
              {field.type === "select" ? (
                <select value={mockFormValues[field.key] || ""} onChange={(event) => dakinisHandleMockFieldChange(field.key, event.target.value)}>
        ...
        <p className="lead">
          Headers efectivos para esta vista:{" "}
          <code>x-business-id={tenantSlugForVertical}</code>
          {" + JWT o maestra "}
