...
export function dakinisValidateConfig(config) {
  ...
  dakinisAssert(
    Array.isArray(config.leads.stages) && config.leads.stages.length > 0,
    "leads.stages debe tener al menos una etapa"
  );
  dakinisAssert(
    typeof config.dashboard.currency === "string" && config.dashboard.currency.length > 0,
    "dashboard.currency es obligatorio"
  );
}
